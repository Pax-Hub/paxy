...
mod nested_building {
    ...
    pub enum PackageNode {
        FlavoredPackage(FlavoredPackage),
        VersionedPackage(VersionedPackage),
    }

    /// Represents a package with possible flavors
    #[derive(Debug, Serialize, Deserialize)]
    pub struct FlavoredPackage {
        #[serde(flatten)]
        pub metadata: PackageMetadata,
        pub flavors: Option<Vec<Package>>,
    }

    /// Represents a leaf package in a tree of flavors
    #[derive(Debug, Serialize, Deserialize)]
    pub struct Package {
        #[serde(flatten)]
        pub metadata: PackageMetadata,
        pub flavors: Option<Vec<PackageFlavor>>,
    }
    ...
    impl Package {
        ...
        pub async fn fetch_flavor(&self, name: &str) -> Option<Flavor> {
            if let Some(url) = self.flavors.get(name) {
                match url.scheme() {
                    "http" | "https" => {
                        let response = reqwest::get(url.clone()).await.unwrap_or_else(|err| {
                            match err.status() {
                                Some(_) => panic!(
            ...
            if let Some(url) = self.versions.get(name) {
                match url.scheme() {
                    "http" | "https" => {
                        ...
                        let text = response.text().await.unwrap();
                        let version =
                            serde_yaml::from_str::<Version>(&text).unwrap_or_else(|err| {
                                panic!(
    ...
    mod tests {
        ...
        #[test]
        fn package_base() {
            let package: Package = serde_yaml::from_str(include_str!(concat!(
                env!("CARGO_MANIFEST_DIR"),
                "/../resources/configuration/manifest/base_package.yaml"
            )))
            .unwrap();
